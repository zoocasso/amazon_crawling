...
def insert_db(url,asin,product_order,product_info,product_detail,product_feature,feature_rating,review_keyword,body_content):
    ...
    cursor.execute(f"select * from amz_category_tb where url like '{url}'")
    ...
    for key in product_detail:
        ...
        cursor.execute(f"""INSERT INTO `amz_product_detail_tb` (url,product_key,product_idx,create_date,detail_table_key,detail_table_value) VALUES("{url}","{checkDictValue_str(product_detail,"ASIN")}","{checkDictValue_str(product_detail_dict,"product_idx")}","{create_date}","{key}","{checkDictValue_str(product_detail,key)}")""")
        mydb.commit()
        index_1 += 1
    ...
    for key in feature_rating:
        ...
        cursor.execute(f"""INSERT INTO `amz_feature_rating_tb` (url,product_key,product_idx,create_date,feature_title,feature_rating) VALUES("{url}","{checkDictValue_str(product_detail,"ASIN")}",{checkDictValue_int(feature_rating_dict,"product_idx")},"{create_date}","{checkDictValue_str(feature_rating_dict,"title")}","{checkDictValue_int(feature_rating_dict,"rating")}")""")
        mydb.commit()
        index_2 += 1
    ...
    for key in product_feature:
        ...
        cursor.execute(f"""INSERT INTO `amz_product_feature_tb` (url,product_key,product_idx,create_date,feature_list_content) VALUES("{url}","{checkDictValue_str(product_detail,"ASIN")}",{checkDictValue_int(product_feature_dict,"product_idx")},"{create_date}","{checkDictValue_str(product_feature_dict,"content")}")""")
        mydb.commit()
        index_3 += 1
    ...
    for key in review_keyword:
        ...
        cursor.execute(f"""INSERT INTO `amz_review_keyword_tb` (url,product_key,product_idx,create_date,keyword) VALUES("{url}","{checkDictValue_str(product_detail,"ASIN")}",{checkDictValue_int(review_keyword_dict,"product_idx")},"{create_date}","{checkDictValue_str(review_keyword,key)}")""")
        mydb.commit()
        index_4 += 1
    ...
    cursor.execute(f"""INSERT INTO `amz_body_content_tb` (url,product_key,create_date,body_content) VALUES("{url}","{checkDictValue_str(product_detail,"ASIN")}","{create_date}","{checkDictValue_str(body_content_dict,"content")}")""")
    mydb.commit()
...
# 상품 정보페이지로 넘어가는 함수
def goToDetailPage(url,asin,product_order):
    # 사이트 스크롤해서 html 불러오기
...
for url in URL_LIST:
    ...
    for asin in asin_list:
        ...
        body_content = list()
        try:
            goToDetailPage(url,asin,product_order)
            product_order += 1
        except:
