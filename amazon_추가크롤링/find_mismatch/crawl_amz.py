...
def insert_db(cursor,url,asin,create_date,product_order,product_info,product_detail,product_feature,feature_rating,review_keyword,body_content):
...
# 상품 정보페이지로 넘어가는 함수
def goToDetailPage(cursor,driver,url,asin,create_date,product_order,product_detail,product_feature,product_info,reviewDict,feature_rating,review_keyword,body_content):
    # 사이트 스크롤해서 html 불러오기
    ...
    insert_db(cursor,url,asin,create_date,product_order,product_info,product_detail,product_feature,feature_rating,review_keyword,body_content)
...
if __name__ =='__main__':
    ...
    for url in URL_LIST:
        ...
        """
            URL별 asin코드 순열 저장
        """
        cursor.execute(f"select * from amz_category_with_asin where url = '{url}'")
        rows = cursor.fetchall()
        row_list = list()
        for row in rows:
            row_dict = dict()
        ...
        for row_dict in row_list:
            ...
            body_content = list()
            # try:
            goToDetailPage(cursor,driver,row_dict['url'],row_dict['asin'],row_dict['create_date'],row_dict['product_idx'],product_detail,product_feature,product_info,reviewDict,feature_rating,review_keyword,body_content)
            # except:
            #     pass
        db.commit()
        db.close()